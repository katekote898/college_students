...
$sql ="Select subject.*, teachers.FIO AS tc_name FROM subject,teachers WHERE 
subject.Id LIKE '%".$Id."%'
AND subject.Name LIKE '%".$Name."%'
AND teachers.FIO LIKE '%".$FIO."%'
AND subject.TeacherId = teachers.Id
 ORDER BY {$sort_sql}";
...
<center> <b style='font-size: 40px; color: #ecf0f1; text-shadow: 2px 2px 4px #000000;'>Предметы</b>
<br>
...
<tr>
  ...
  <th><?php echo sort_link_th('Преподаватель', 'FIO_asc', 'FIO_desc');?></th>
  <th></th>
</tr>
...
echo "<td>     
                <!-- Форма удаления записи -->
                   ...
                   <a href='upd_subject.php?Id=" . $row["Id"] . "'>Изменить</a>
      </td>";
echo "</tr>";
...
<form class='form_add' action='add_subject.php' method='POST' enctype='multipart/form-data'>
  <table class='form_add'>
    <th>Добавление</th>
    <tr>
      <td>Id</td>
